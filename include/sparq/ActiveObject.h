...
#ifndef SPARQ_ACTIVEOBJECT_H
#define SPARQ_ACTIVEOBJECT_H
...
namespace sparq {
    ...
    protected:
        ...
        timerid_t addTimer(uint64_t micros, const callback_t &callback) {
            return this->timers.add(mkTimeStamp(micros, callback));
        }

        // This version allows you to post a command at the given time
        timerid_t addTimer(uint64_t micros, const MsgType &msg) {
